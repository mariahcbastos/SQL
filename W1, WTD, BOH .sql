  ...
  (
   ...
   FROM
     (
      ...
      LEFT JOIN (
         SELECT DISTINCT inventory_date, (SELECT max(inventory_date) FROM dsipc.dc_inventory) latestinventory
         FROM
          dsipc.dc_inventory
...
LEFT JOIN (
   SELECT
   im.asdcitemid, "inventory_status"  , "max_allowed_qty"  , "inventory_date", "firstdayofweek", "total_wtd_sales_qty"
...
--GROUP by invoices_week_start_date, invoices.gtin, cm.dcextid
...
--GROUP by invoices.gtin,invoices.asdcitemid, invoices_week_start_date, invoices.invoice_date
